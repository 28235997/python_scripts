...
def parse_xml_attributes_and_decode(xml_file):
    tree = ET.parse(xml_file)
    root = tree.getroot()

    print("🔍 Extracting attributes and decoding Base64 values...\n")
    ...
    for element in root.iter():
        for attr_key, attr_value in element.attrib.items():
            ...
            try:
                decoded_value = base64.b64decode(attr_value).decode("utf-8")
            except Exception:
                decoded_value = "<invalid base64 or plain text>"

            print(f"🗝️ Attribute : {attr_key}")
            print(f"📥 Value     : {decoded_value}\n")


# Example usage
if __name__ == "__main__":
    xml_path = "test.xml"   # your xml file
    parse_xml_attributes_and_decode(xml_path)
